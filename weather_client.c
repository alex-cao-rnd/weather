/*******************************************************************************************************
** 名称：天气HTTP客户端（Linux版本）
** 作者：正念君
** 作者微信公众号：嵌入式大杂烩
** 编译命令：gcc -std=c99 weather_client.c cJSON.c -o weather_client
...
/* 秘钥，注意！！如果要用这一份代码，这个一定要改为自己的，因为这个我已经故意改错了，防止大家与我公用一个KEY */
...
static void GetWeather(char *weather_json, char *location, Weather *result)
{
	...
	/* 接受服务端的返回数据 */
	read(ClientSock, WeatherRecvBuf, 2*1024);
